...
from deepstochlog.term import List
...
def calculate_zipped_probabilities(
    ...
    probabilities = model.predict_sum_product(batch=possibilities, model_eval=True, test_dbs=test_dbs)
        ...
        with torch.no_grad():
            for batch in test_data:
                ...
                for elem in batch:
                    ...
                    other_possibilities[
                        elem
                    ] = model.get_direct_contextualized_proof_possibilities(elem, mask_index=mask_index, mask_term=mask_term, process_query=process_query)
                ...
                for possibility, prob in calculate_zipped_probabilities(
                    model, all_to_evaluate, test_dbs = test_dbs
                ):
                    probabilities[possibility] = prob

                for elem in batch:

                    other_parse = other_possibilities[elem]

                    parsed_elem = copy.deepcopy(elem)

                    parsed_elem.term = Term(
                        elem.term.functor,
                        Term(elem.term.arguments[0].functor.replace("'", "")),
                        Term(elem.term.arguments[1].functor.replace("'", "")),
                        List(*[Term(token.functor.replace("'", "")) for token in elem.term.arguments[2]]),
                    )

                    own_prob = probabilities[parsed_elem]
                    label_probability += own_prob

                    is_highest = not any(
                        [
                            pos
                            for pos in other_parse
...
def create_model_accuracy_calculator(
    ...
    def calculate_model_accuracy(mask_index=0, mask_term=None, process_query=False, test_dbs=None) -> str:
        val_acc = None
