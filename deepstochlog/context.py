...
class ContextualizedTerm:
    ...
    def __hash__(self):
        res = hash((self.context, self.term))
        return res
        # return hash((self.context, self.term))

    def mask_generation_output(self, mask_index=0):
        return ContextualizedTerm(
            term=self.term.mask_generation_output(mask_index),
            context=self.context,
        )

    def calculate_query(
        self, masked_generation_output=False, mask_index=0, limit=None
    ) -> Term:
        """ Calculates which queries are necessary to ask Prolog based on which terms are in the dataset """
        if masked_generation_output:
            query = self.term.mask_generation_output(mask_index)
        else:
            query = self.term
        return query
