...
def create_rules_parser():
    ...
    constants = alphas + nums + "_" + "\'" + "*" + " " + "%"
    constants_word = Word(constants)

    # forward for nested / recursive structure
    term_forward = Forward()
    term = constants_word + Optional(Suppress("(") + Group(delimitedList(OneOrMore(Group(term_forward)) + Optional(
        Suppress(",") + Suppress("[") + Optional(Group(delimitedList(Group(term_forward))) + Suppress("]")))).setResultsName(
        "arguments")) + Suppress(")"))
    term_forward << term
...
def parse_term(input_term: ParseResults) -> Term:
    if not isinstance(input_term[0], str):
        return [parse_term(t) for t in input_term]
    else:
