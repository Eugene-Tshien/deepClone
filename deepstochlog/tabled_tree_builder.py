...
def parse_list(input_list):
    ...
    for elem in input_list:
        if elem == "[|]":
            ...
            if elem[0] == "[|]":
                processed.extend(parse_list(elem))
            else:
                processed.append(elem)
        else:
            print("Unexpected element in prolog solving lists")
    return processed
...
class TabledAndOrTreeBuilder:
    def __init__(
        ...
        # Todo: better way to handle singleton warning
        self.prolog_program = ":- style_check(-singleton).\n" + self.prolog_program
    ...
    def build_and_or_trees(
        ...
        def ground_all(input_terms: typing.Iterable[Term]):
            ...
            # print("Solving the program took {:.3} seconds".format(solve_time))
            ...
            for k, raw_line in enumerate(prolog_result):
                ...
                long_list = parent_raw[-2]
                diff_list = parent_raw[-1]
                if "[|]" in long_list:
                    parent_raw[-2] = ["pylist"]+parse_list(long_list)
                if "[|]" in diff_list:
                    parent_raw[-1] = ["pylist"]+parse_list(diff_list)
                conjunction_raw = line_parts[1]
                for elem in conjunction_raw:
                    if isinstance(elem, list) and not elem[0]=="p" and not elem[0]=="nn":
                        long_list = elem[-2]
                        diff_list = elem[-1]
                        if "[|]" in long_list:
                            elem[-2] = ["pylist"] + parse_list(long_list)
                        if "[|]" in diff_list:
                            elem[-1] = ["pylist"] + parse_list(diff_list)
